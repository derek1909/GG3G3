...
def classifier_tester(M=100, N=400, T=100, classifier="var", thresholds=3):
    ...
    accuracies = np.zeros((3,3)) # accuracies[0,:] is for first sigma range
    r_list = [-1, 0, 1]
    s_list = [-1, 0, 1]
    ...
    for ii in [0, 1, 2]: ## for sigma
        for jj in [0, 1, 2]: ## for r
            ...
            data_points = generate_spike_trains(M=M, N=N, T=T, rmin=rmin, rmax=rmin+1, bmin=-1, bmax=1, smin=smin, smax=smin+1, mmin=10, mmax=90, xmin=0.05, xmax=0.95)
            ...
            accuracies[ii,jj] = accuracy    
    return accuracies

def generate_spike_trains(M=20, N=400, T=100, rmin=0, rmax=1, bmin=0.5, bmax=2, smin=0.1, smax=1, mmin=10, mmax=90, xmin=0, xmax=1):
    """
